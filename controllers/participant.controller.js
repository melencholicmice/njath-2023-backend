...
export const getQuestion = async (req, res) => {
    ...
    try {
        ...
        if (response) {
            ...
            if (currUser.points < config.questionOpenPenelty) {
                return res.status(401).json({
                    success: false,
                    message:
                        'You dont have enough points to perform this action',
                })
            }
        ...
        const question = await Question.findOne({
            ...
            .select('-_id title description imageUrl level order')
            .exec()
...
export const getLevelDetails = async (req, res) => {
    ...
    try {
        const correctQuestions = await QuestionResponse.find({
            ...
            .select('-_id order isCorrect hintTaken')
            .exec()
        ...
        const isSolved =
            config.maxQuestioninLevel === questionSolved ? true : false
        return res.status(200).json({
            success: true,
            data: {
                question: questions,
        ...
        if (safe != 0) {
            if (currUser.points > config.loanPenalty) {
                return res.status(401).json({
                    success: false,
                    message:
                        "You have enough points , you don't need to take loan",
                })
            }
        }
...
export const getLeaderBoard = async (req, res) => {
    try {
        ...
        participants = participants.slice(0, 500)

        let rank =
            participants.findIndex((p) => p.username === participantId) + 1
